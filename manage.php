<?php
// ======================================================================
// PPC Portal - Backend Logic (v2 with personalization)
// ======================================================================
session_start();
...
function initialize_database() {
    if (!file_exists(DB_PATH)) {
        $db = get_db();
        // Updated Schema with origin, source, and quantity
        $query = "
...
function delete_entry_files($entry) {
    if ($entry) {
        ...
        // Delete image file (if not the placeholder)
...
function regenerate_all() {
    ...
    // Clear old entry files before regenerating
...
// --- [REPLACE] `regenerate_index` function ---
...
<body>
        ...
        <div id="ppcGrid" class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-4">
            ...
            <?php 
                $search_content = strtolower(implode(' ', [
                    $entry['title'], $entry['location'], $entry['year'], $entry['region'],
                    $entry['type'], $entry['origin'], $entry['source']
                ]));
            ?>
    ...
    <footer class="text-center py-4 bg-dark text-white mt-5">
        <p class="mb-0">© <?= date('Y') ?> <?= OWNER_NAME ?>. A catalog of my private collection.</p>
    </footer>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupPagination(items) {
        ...
        if (pageCount <= 1) {
            resultsCount.textContent = `${items.length} entr${items.length === 1 ? 'y' : 'ies'} found.`;
            return;
        }
        resultsCount.textContent = `Showing ${Math.min(items.length, ITEMS_PER_PAGE * currentPage)} of ${items.length} entries.`;
        const prevLi = document.createElement('li');
        prevLi.className = `page-item ${currentPage === 1 ? 'disabled' : ''}`;
        prevLi.innerHTML = `<a class="page-link">Previous</a>`;
        prevLi.addEventListener('click', () => { if (currentPage > 1) showPage(currentPage - 1, items); });
        paginationContainer.appendChild(prevLi);
        for (let i = 1; i <= pageCount; i++) {
            const li = document.createElement('li');
            li.className = `page-item ${i === currentPage ? 'active' : ''}`;
            li.innerHTML = `<a class="page-link">${i}</a>`;
            li.addEventListener('click', () => showPage(i, items));
            paginationContainer.appendChild(li);
        }
        const nextLi = document.createElement('li');
        nextLi.className = `page-item ${currentPage === pageCount ? 'disabled' : ''}`;
        nextLi.innerHTML = `<a class="page-link">Next</a>`;
        nextLi.addEventListener('click', () => { if (currentPage < pageCount) showPage(currentPage + 1, items); });
        paginationContainer.appendChild(nextLi);
    }
    function updatePaginationUI(totalItems) {
        const pageCount = Math.ceil(totalItems / ITEMS_PER_PAGE);
        const pageItems = paginationContainer.children;
        ...
        pageItems[pageItems.length - 1].classList.toggle('disabled', currentPage === pageCount);
    }
    function handleSearch() {
        const query = searchInput.value.toLowerCase().trim();
        filteredCards = allCards.filter(card => card.getAttribute('data-search-content').includes(query));
...
// --- [REPLACE] `regenerate_entry_pages` function ---
...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark sticky-top">
        <div class="container">
            <a class="navbar-brand" href="../index.html"><i class="bi bi-envelope-paper-heart-fill"></i> PPC Catalog</a>
        </div>
    </nav>
    <main class="container my-5">
        <div class="row g-5">
            <div class="col-lg-5">
                <img src="../assets/images/<?= htmlspecialchars($entry['image']) ?>" class="img-fluid rounded shadow-sm" alt="<?= htmlspecialchars($entry['title']) ?>" style="width: 100%; height: auto; aspect-ratio: 4/3; object-fit: cover;">
            </div>
            <div class="col-lg-7">
                ...
                <dl class="row">
                    ...
                    <dd class="col-sm-9"><span class="badge bg-primary"><?= htmlspecialchars($entry['origin']) ?></span></dd>
                    
                    <dt class="col-sm-3">Year</dt>
                    <dd class="col-sm-9"><?= htmlspecialchars($entry['year']) ?></dd>

                    <dt class="col-sm-3">Region</dt>
                    <dd class="col-sm-9"><?= htmlspecialchars($entry['region']) ?></dd>

                    <dt class="col-sm-3">Type</dt>
                    <dd class="col-sm-9"><span class="badge bg-info text-dark"><?= htmlspecialchars($entry['type']) ?></span></dd>

                    <dt class="col-sm-3">Quantity</dt>
                    <dd class="col-sm-9"><?= htmlspecialchars($entry['quantity']) ?></dd>

                    <?php if (!empty($entry['source'])): ?>
                    <dt class="col-sm-3">Source</dt>
                    <dd class="col-sm-9"><?= htmlspecialchars($entry['source']) ?></dd>
                    <?php endif; ?>
                </dl>

                <h3 class="h5 mt-4">Description</h3>
                <div class="description bg-light p-3 rounded">
                    <p class="mb-0"><?= nl2br(htmlspecialchars($entry['description'])) ?></p>
                </div>
    ...
    <footer class="text-center py-4 bg-body-tertiary mt-5 border-top">
        <p class="mb-0">© <?= date('Y') ?> <?= OWNER_NAME ?>. A catalog of my private collection.</p>
    </footer>
</body>
</html>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'create' || $action === 'edit') {
            // Retrieve all form fields, including new ones
            ...
            if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                ...
                move_uploaded_file($tmp_name, IMAGES_DIR . $image_name);
            }
            ...
            if ($action === 'create') {
                ...
                if ($image_name !== $oldEntry['image'] && $oldEntry['image'] !== 'placeholder.jpg') {
                    if (file_exists(IMAGES_DIR . $oldEntry['image'])) {
                        unlink(IMAGES_DIR . $oldEntry['image']);
                    }
                }
            }
...
<head>
    ...
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.1/font/bootstrap-icons.css">
    <style>
        .live-preview { border: 1px dashed #ccc; min-height: 150px; }
    </style>
</head>
<body>
                ...
                <form method="POST" enctype="multipart/form-data" class="mb-5">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="title" name="title" value="<?= htmlspecialchars($edit_entry['title'] ?? '') ?>" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="location" class="form-label">Location</label>
                            <input type="text" class="form-control" id="location" name="location" value="<?= htmlspecialchars($edit_entry['location'] ?? '') ?>" required>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-3 mb-3">
                            ...
                            <select class="form-select" id="origin" name="origin">
                                <?php $origins = ['India', 'International']; ?>
                                <?php foreach($origins as $o): ?>
                                <option value="<?= $o ?>" <?= (($edit_entry['origin'] ?? 'India') == $o) ? 'selected' : '' ?>><?= $o ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-3 mb-3">
                            ...
                            <select class="form-select" id="type" name="type">
                                <?php $types = ['Permanent', 'Special', 'Commemorative', 'Other']; ?>
                                <?php foreach($types as $type): ?>
                                <option value="<?= $type ?>" <?= (($edit_entry['type'] ?? '') == $type) ? 'selected' : '' ?>><?= $type ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-3 mb-3">
                            <label for="year" class="form-label">Year</label>
                            <input type="number" class="form-control" id="year" name="year" placeholder="e.g., 2023" value="<?= htmlspecialchars($edit_entry['year'] ?? '') ?>" required>
                        </div>
                        <div class="col-md-3 mb-3">
                            <label for="quantity" class="form-label">Number of Copies</label>
                            <input type="number" class="form-control" id="quantity" name="quantity" min="1" value="<?= htmlspecialchars($edit_entry['quantity'] ?? '1') ?>" required>
                        </div>
                    </div>
                    
                    <div class="row">
                         <div class="col-md-6 mb-3">
                            <label for="region" class="form-label">Region (e.g., State)</label>
                            <input type="text" class="form-control" id="region" name="region" value="<?= htmlspecialchars($edit_entry['region'] ?? '') ?>">
                        </div>
                         <div class="col-md-6 mb-3">
                            <label for="source" class="form-label">Source (Where you got it from)</label>
                            <input type="text" class="form-control" id="source" name="source" placeholder="e.g., eBay, GPO Bangalore, Trade" value="<?= htmlspecialchars($edit_entry['source'] ?? '') ?>">
                        </div>
                    </div>

                    <div class="mb-3">
                        <label for="description" class="form-label">Description</label>
                        <textarea class="form-control" id="description" name="description" rows="4"><?= htmlspecialchars($edit_entry['description'] ?? '') ?></textarea>
                    </div>

                    <div class="mb-3">
                        <label for="image" class="form-label">Image</label>
                        <input class="form-control" type="file" id="image" name="image">
                        <?php if ($edit_entry && $edit_entry['image']): ?>
                            <small class="form-text text-muted">Current: <?= $edit_entry['image'] ?>. Upload new file to replace.</small>
                        <?php endif; ?>
                    </div>
                    
                    <button type="submit" class="btn btn-primary"><?= $edit_entry ? 'Update Entry' : 'Create Entry' ?></button>
                    <?php if ($edit_entry): ?>
                        <a href="manage.php" class="btn btn-secondary">Cancel Edit</a>
                    <?php endif; ?>
                </form>
            </div>
        </div>
            ...
            <table class="table table-striped table-hover align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="entries/<?= $entry['id'] . '-' . $entry['slug'] ?>.html" class="btn btn-sm btn-info" title="View Static Page" target="_blank"><i class="bi bi-eye"></i></a>
                            <a href="?action=edit&id=<?= $entry['id'] ?>" class="btn btn-sm btn-warning" title="Edit"><i class="bi bi-pencil"></i></a>
                            <form method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this entry? This cannot be undone.');">
