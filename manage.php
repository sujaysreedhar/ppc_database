...
<head>
    ...
    <style>
        ...
        .ppc-card.d-none { display: none !important; }
    </style>
</head>
<body>
...
document.addEventListener('DOMContentLoaded', function() {
    const ITEMS_PER_PAGE = 12; // Items to show per page
    ...
    function showPage(page, items) {
        ...
        // Hide all items first by adding d-none class
        allCards.forEach(card => card.classList.add('d-none'));
        
        // Show only the items for the current page by removing d-none class
    ...
    function setupPagination(items) {
        ...
        resultsCount.textContent = `Showing ${items.slice(0, ITEMS_PER_PAGE * currentPage).length} of ${items.length} entries.`;
        ...
        prevLi.addEventListener('click', () => {
            if (currentPage > 1) showPage(currentPage - 1, items);
        });
        ...
        nextLi.addEventListener('click', () => {
            if (currentPage < pageCount) showPage(currentPage + 1, items);
        });
    ...
    function handleSearch() {
        ...
        filteredCards = allCards.filter(card => {
            return card.getAttribute('data-search-content').includes(query);
        });
        
        showPage(1, filteredCards); // Go to first page of results
        setupPagination(filteredCards); // Re-create pagination for filtered results
    }
