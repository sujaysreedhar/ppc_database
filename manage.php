...
<body>
    <main class="page">
        <article class="content">
            <header class="page-header">
                <a class="back-link" href="../index.html">&larr; Back to full collection</a>
                <h1><?= htmlspecialchars($entry['title']) ?></h1>
                <?php if (!empty($entry['location'])): ?>
                <div class="subtitle"><?= htmlspecialchars($entry['location']) ?></div>
                <?php endif; ?>
            </header>
            <aside class="infobox">
                <table class="infobox-table">
                    ...
                    <tr class="infobox-image">
                        <td colspan="2"><img src="../assets/images/<?= htmlspecialchars($entry['image']) ?>" alt="<?= htmlspecialchars($entry['title']) ?>"></td>
                    </tr>
                    <tr>
            ...
            <div class="trade-notice">
                Available for trade: <?= htmlspecialchars($entry['quantity'] - 1) ?> extra cop<?= ($entry['quantity'] - 1) > 1 ? 'ies' : 'y' ?>.
                <a class="trade-link" href="<?= htmlspecialchars($trade_link) ?>" target="_blank" rel="noopener">Make a trade offer</a>.
            </div>
            <?php endif; ?>
            <section>
                <h2>Description</h2>
    ...
    <footer>© <?= date('Y') ?> <?= OWNER_NAME ?></footer>
</body>
</html>
<?php
        $html = ob_get_clean();
